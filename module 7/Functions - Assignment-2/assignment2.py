...
# By a fixed monthly payment, we mean a single number which does not c
#hange each month, but instead is a constant amount that will be
# paid each month.
...
def payingdebtoffinayear(balance, annualinterestrate):
    ...
    while blnce > 0:
        ...
        while month <= 12:
            ...
            monthlyunpaidbalance = (blnce) - (monthlypayment)
            updatedbalance = (monthlyunpaidbalance) + (monthlyinterest * monthlyunpaidbalance)
            blnce = updatedbalance
            month += 1
